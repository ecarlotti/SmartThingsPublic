 ...
 *  Add-ons by Scott Barton @ SmartThings Community (https://community.smartthings.com/t/zwn-sc7-enerwave-7-button-scene-controller/2969/707)
 *
 */
definition(
    name: "ZWN-SC7 SmartApp",
    namespace: "ecarlotti",
    author: "Edson Carlotti using code from Scott Barton, using code from Eric M, using code from Matt Frank, using code from Brian Dahlem",
...
def getButtonSections(buttonNumber) {
  return {
   ...
   //SB
    ...
    section(title: "Adjust Dimmer Up 20%", hidden: hideSection(buttonNumber, "dimmerUp"), hideable: true) {
      input "dimmers_${buttonNumber}_dimmerUp", "capability.switchLevel", title: "Dimmers:", multiple: true, required: false
    }
    section(title: "Adjust Dimmer Down 20%", hidden: hideSection(buttonNumber, "dimmerDown"), hideable: true) {
      input "dimmers_${buttonNumber}_dimmerDown", "capability.switchLevel", title: "Dimmers:", multiple: true, required: false
    }
    section(title: "Cycle Dimmer Through Values", hidden: hideSection(buttonNumber, "dimmerCycle"), hideable: true) {
      input "dimmers_${buttonNumber}_dimmerCycle", "capability.switchLevel", title: "Dimmers:", multiple: true, required: false
      input "dimmers_${buttonNumber}_dimmerLevels", "number", title: "Dimmer Levels", required: false, description: "list of 0 to 99 separated by comma"
    }    
...
def buttonConfigured(idx) {
  return settings["lights_$idx_toggle"] ||
         ...
         settings["dimmers_$idx_dimmerCycle"] ||
         settings["dimmers$idx_dimmerLevels"] ||
...
/**********************************************************************************************
23/12/2020 (ELC) - Disabled the debounce - It was causing the app not to work at all
...
//SB
  ...
  dimmers = find('dimmers', buttonNumber, "dimmerCycle")
  def levels = find('dimmers', buttonNumber, "dimmerLevels")
  if (dimmers != null) dimmerCycle(dimmers, levels)
...
def dimmerCycle(devices, levels) {
    //Must take your time (~5s) between button presses or it may get messed up. 
    if (levels == null) levels = "0,35,55,85"
	log.debug "dimmerCycle: $devices = ${devices*.currentLevel}"
    log.debug "dimmerCycle: levels = ${levels}"
    
    def levelsList = devices*.currentLevel
    levelsList.sort()
    levelsList = levelsList.reverse()
	def currentLevel = levelsList[0]
   	log.debug "dimmerCycle: currentLevel = $currentLevel"

	// Get the next level from the levels list
    def cycleLevels = levels.split(",")
    int idx = 0
    while ((idx < cycleLevels.size()) && (Integer.parseInt(cycleLevels[idx]) <= currentLevel)) {
    	idx++
    }
    if (idx >= cycleLevels.size()) idx = 0;
    int nextLevel = cycleLevels[idx] as Integer;
    log.debug "dimmerCycle: nextLevel = $nextLevel"
